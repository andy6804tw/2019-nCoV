...
var _mqtt = require('../modules/mqtt.module');
...
const dataGet = (req, res) => {
  ...
  _mqtt2.default.selectAllData(macaddr, startTime, endTime).then(result => {
...
const dataPost = (req, res) => {
  ...
  _mqtt2.default.createData(insertValues).then(result => {
    res.send(result); // 成功回傳result結果
  }).catch(err => {
    return res.send(err);
  }); // 失敗回傳錯誤訊息
