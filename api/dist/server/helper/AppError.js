...
class APIError extends ExtendableError {
   ...
   * @memberof APIError
   */
  constructor(status = _httpStatus2.default.INTERNAL_SERVER_ERROR, message, tag, code) {
...
class MySQLError extends ExtendableError {
   ...
   * @memberof MySQLError
   */
  constructor(status = _httpStatus2.default.INTERNAL_SERVER_ERROR, message = '網路連線不穩，請稍後再試', tag = 'SERVER_ERROR', code = 500) {
    super(status, message, tag, code);
...
class LoginError1 extends ExtendableError {
   ...
   * @memberof MySQLError
   */
  constructor(status = _httpStatus2.default.UNAUTHORIZED, message = '信箱尚未註冊！', tag = 'Account_ERROR', code = 401) {
    super(status, message, tag, code);
...
class LoginError2 extends ExtendableError {
   ...
   * @memberof MySQLError
   */
  constructor(status = _httpStatus2.default.UNAUTHORIZED, message = '您輸入的密碼有誤！', tag = 'Account_ERROR', code = 401) {
    super(status, message, tag, code);
    this.name = 'LoginError';
  }
}
...
class DownloadError extends ExtendableError {
   ...
   * @memberof MySQLError
   */
  constructor(status = _httpStatus2.default.NOT_FOUND, message = '您尚未有上傳檔案的紀錄！', tag = 'Download_ERROR', code = 404) {
    super(status, message, tag, code);
